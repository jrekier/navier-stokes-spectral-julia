...
struct SimulationParams
    kx::CuArray{Float64, 2}
    ky::CuArray{Float64, 2}
    kmax::Float64
    kSq::CuArray{Float64, 2}
    kSq_inv::CuArray{Float64, 2}
    dealias::CuArray{Bool, 2}
end
...
function poisson_solve(rho::CuArray{Float64, 2}, params::SimulationParams)
    V_hat = -fft(rho) .* params.kSq_inv
    V = real(ifft(V_hat))
    return V
end

function diffusion_solve(v::CuArray{Float64, 2}, dt::Float64, nu::Float64, params::SimulationParams)
    v_hat = fft(v) ./ (1.0 .+ dt * nu .* params.kSq)
    v = real(ifft(v_hat))
    return v
end

function grad(v::CuArray{Float64, 2}, params::SimulationParams)
    v_hat = fft(v)
...
function div(vx::CuArray{Float64, 2}, vy::CuArray{Float64, 2}, params::SimulationParams)
    dvx_x = real(ifft(1im * params.kx .* fft(vx)))
    dvy_y = real(ifft(1im * params.ky .* fft(vy)))
    return dvx_x + dvy_y
end

function curl(vx::CuArray{Float64, 2}, vy::CuArray{Float64, 2}, params::SimulationParams)
    dvx_y = real(ifft(1im * params.ky .* fft(vx)))
    dvy_x = real(ifft(1im * params.kx .* fft(vy)))
    return dvy_x - dvx_y
end

function apply_dealias(f::CuArray{Float64, 2}, params::SimulationParams)
    f_hat = params.dealias .* fft(f)
    return real(ifft(f_hat))
end

function time_step(vx::CuArray{Float64, 2}, vy::CuArray{Float64, 2}, dt::Float64, nu::Float64, params::SimulationParams)
